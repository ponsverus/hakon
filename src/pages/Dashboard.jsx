...
import { 
  Plus, Edit, Trash2, X, ExternalLink, Eye, Copy, Check, Calendar, DollarSign, 
...
export default function Dashboard({ user, onLogout }) {
    ...
    try {
      ...
      if (profissionaisData?.length > 0) {
        ...
        if (agendamentosData) {
          ...
          // Recarregar após auto-conclusão
  ...
  const createServico = async (e) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from('servicos').insert([formServico]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-primary/10 border border-primary/30 rounded-custom p-6 mb-8">
          ...
          <div className="flex gap-2">
            <input type="text" value={`${window.location.origin}/v/${barbearia.slug}`} readOnly className="flex-1 px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white text-sm" />
        ...
        <div className="bg-dark-100 border border-gray-800 rounded-custom overflow-hidden">
          <div className="flex overflow-x-auto border-b border-gray-800">
            {['visao-geral', 'agendamentos', 'cancelados', 'servicos', 'profissionais'].map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-shrink-0 px-6 py-4 font-black text-sm transition-all capitalize ${activeTab === tab ? 'bg-primary/20 text-primary border-b-2 border-primary' : 'text-gray-400 hover:text-white'}`}>
                {tab.replace('-', ' ')}
              </button>
            ))}
          </div>
          ...
          <div className="p-6">
            ...
            {activeTab === 'cancelados' && (
              <div>
                ...
                {agendamentos.filter(a => a.status.includes('cancelado')).length > 0 ? (
                  <div className="space-y-4">
                    {agendamentos.filter(a => a.status.includes('cancelado')).map(a => (
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {servicos.map(s => (
                      <div key={s.id} className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => { setEditingServico(s.id); setFormServico({ ...s }); }} className="flex-1 py-2 bg-blue-500/20 border border-blue-500/50 text-blue-400 rounded-custom font-bold text-sm">
                            Editar
                          </button>
                    ...
                    <div key={p.id} className="bg-dark-200 border border-gray-800 rounded-custom p-5">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          ...
                          {p.anos_experiencia && <p className="text-xs text-gray-500 font-bold">{p.anos_experiencia} anos</p>}
                        </div>
                      </div>
                      ...
                      <button onClick={() => { setEditingProfissional(p); setFormProfissional({ nome: p.nome, anos_experiencia: p.anos_experiencia, horario_inicio: p.horario_inicio, horario_fim: p.horario_fim }); }} className="w-full py-2 bg-blue-500/20 border border-blue-500/50 text-blue-400 rounded-custom font-bold text-sm">
                        Editar Horários
                      </button>
                    </div>
                  ))}
          ...
          <div className="bg-dark-100 border border-gray-800 rounded-custom max-w-md w-full p-8">
            ...
            <form onSubmit={createServico} className="space-y-4">
              <div>
                ...
                <select value={formServico.profissional_id} onChange={(e) => setFormServico({ ...formServico, profissional_id: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" required>
              ...
              <div>
                ...
                <input type="text" value={formServico.nome} onChange={(e) => setFormServico({ ...formServico, nome: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
              </div>
              <div>
                <label className="block text-sm font-bold mb-2">Duração (min)</label>
                <input type="number" value={formServico.duracao_minutos} onChange={(e) => setFormServico({ ...formServico, duracao_minutos: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
              </div>
              <div>
                <label className="block text-sm font-bold mb-2">Preço (R$)</label>
                <input type="number" step="0.01" value={formServico.preco} onChange={(e) => setFormServico({ ...formServico, preco: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
              </div>
            ...
            <form onSubmit={createProfissional} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="time" value={formProfissional.horario_inicio} onChange={(e) => setFormProfissional({ ...formProfissional, horario_inicio: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
                </div>
                <div>
                  <label className="block text-sm font-bold mb-2">Fim</label>
                  <input type="time" value={formProfissional.horario_fim} onChange={(e) => setFormProfissional({ ...formProfissional, horario_fim: e.target.value })} className="w-full px-4 py-3 bg-dark-200 border border-gray-800 rounded-custom text-white" required />
                </div>
              </div>
              <button type="submit" className="w-full py-3 bg-gradient-to-r from-primary to-yellow-600 text-black rounded-button font-black">
                ADICIONAR
